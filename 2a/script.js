...
"1-3 a: abcdeaaaaa",
"1-3 b: cdefg",
"2-9 c: cccccccccccccc"
] */

// mock input samo ene vrstice:
...
function preveriGeslo (inputString) {
    ...
    // zadetki regexov
    minNum = minNumRgx.exec(inputString)[0]
    maxNum = inputString.match(maxNumRgx)[1]
    character = inputString.match(characterRgx)[1]
    stringToSearch = inputString.match(stringToSearchRgx)[1]

    // pri characterju potrebno dodati global flag, da ga lahko potem večkrat prešteje v naslednji vrstici
    let characterRgxWithActualLetter = new RegExp(character,"g")
    let numOfOccurances = (stringToSearch.match(characterRgxWithActualLetter)||[]).length
    // v zg. vrstici ||[] potreben za primere, ko ni zadetkov

    // izpis vmesnih korakov v konzoli
    /* 
    ...
    console.log("\nštevilo pojavitev črke:", numOfOccurances) 
    */

    // preveri če geslo ustrezno:
    if ((numOfOccurances >= minNum) && (numOfOccurances <=maxNum)) {
        // console.log ("Geslo JE ok" )
        counter += 1
    }
    else {
        // (console.log("Geslo NI ok"))
    }
}
