...
// mock line input
// let myString = "1-3 a: bcdeaa"

//function zapisiRezultat () {addEventListener(click)}

var counter = 0

function preveriGeslo (InputString) {
    
    //regexi
    let firstPlaceRgx = /\d+/
    let secondPlaceRgx = /-(\d+)/ 
    let characterRgx = /\d+-\d+ (\w):/
    let stringToSearchRgx = /\d+-\d+ \w: (\w+)/

    // zadetki regexov
    firstPlace = firstPlaceRgx.exec(InputString)[0]
    secondPlace = InputString.match(secondPlaceRgx)[1]
    character = InputString.match(characterRgx)[1]
    stringToSearch = InputString.match(stringToSearchRgx)[1]

    // spodnji izraz pomeni XOR  toda ker JS nima običajnega XOR izraza, se ga dobi takole 
    if (!(stringToSearch[firstPlace-1] === character) != !(stringToSearch[secondPlace-1] === character)) {
        counter +=1
    }

    // izpis posameznih korakov v Konzoli: (da videl, če pravilno deluje na mock primeru)
    /* 
    console.log("\nINPUT STRING:", InputString)
    console.log("prva pozicija: ", firstPlace)
    console.log("druga: ", secondPlace)
    console.log("črka: ", character)
    console.log("string za preiskavo: ", stringToSearch)
    
    if (stringToSearch[firstPlace-1] === character) {
        console.log("prva OK")
    }
    else {
        console.log("prva NE ujema")
    }

    if (stringToSearch[secondPlace-1] === character) {
        console.log("druga je OK")
    }
    else console.log("druga NE ujema")


    if ((stringToSearch[firstPlace-1] === character) || (stringToSearch[secondPlace-1] === character)) {
        console.log("celo geslo je pravilno")
    }
    else {
        console.log("celo geslo je nepravilno")
    }
 */
}
...
console.log ("Število pravilnih gesel: ", counter)
