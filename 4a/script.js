...
let neccesaryFields = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
let validPassportCounter = 0

for (passport of inputList) {
    console.log("NOVI PASSPORT")
    numOfRequiredFields = 0
    let isValid = true
    for (field of neccesaryFields) {
        if (isValid === true) {     // ? a je kak elegantnejši način za breakat outer loop?
            if (passport.includes(field)){
                console.log("OK")
            }
            else {
                console.log("manjka field")
                isValid = false
            }
        }    
    }
...
console.log("st validnih passportov:", validPassportCounter)