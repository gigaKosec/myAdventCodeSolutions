...
// spremeni input string v array 
let inputArray = (input.split("\n\n"))
let inputArrayWithSubarrays = []
for (let i of inputArray) {
    inputArrayWithSubarrays.push(i.split("\n"))
}


let unanimousDecisionsForEachGroup = []

for (let group of inputArrayWithSubarrays) {
    console.log("\nSkupina:", group)
    let groupCounters = {}
    
    for (let choicesOfPerson of group){
        console.log("Oseba:")
        for (let choice of choicesOfPerson) {
            console.log("izbira:",choice)
            if (choice in groupCounters) {
                groupCounters[choice] +=1
            } else {
                groupCounters[choice] = 1
            }
        }
    ...
    console.log("Števci: ",groupCounters)

    let choicesOfAll = []
    for (let choice in groupCounters){
        if (groupCounters[choice] === group.length){
            choicesOfAll.push(choice)         
        }
    }
    console.log("skupne izbire:", choicesOfAll)
    unanimousDecisionsForEachGroup.push(choicesOfAll)
}
console.log("enoglasne odločitve po skupinah:", unanimousDecisionsForEachGroup)

let totalUnanimousDecisionsCounter = 0
for (let group of unanimousDecisionsForEachGroup){
    totalUnanimousDecisionsCounter += group.length
}
console.log("št. vseh skupnih odločitev =", totalUnanimousDecisionsCounter)